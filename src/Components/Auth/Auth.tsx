...
export default function Auth() {
	...
	const { isAuthClicked, setIsAuthClicked, loginBtn, setLoginBtn } = useGlobalContext()

	return (
		<div
			className={`${isAuthClicked ? "scale-100 opacity-100" : "scale-0 opacity-0"
				} overflow-scroll fixed top-0 left-0 flex items-center justify-center bg-black bg-opacity-30 w-screen h-screen`}
		>
			<div className={`${isAuthClicked ? "scale-100 opacity-100" : "scale-0 opacity-0"
				...
				<header className='h-12 w-[95%] border-b'>
					...
					<button onClick={() => setLoginBtn(true)} className={`${loginBtn ? 'auth-btn-active' : null} auth-btn rounded-l-md`}>Login</button>

					<button onClick={() => setLoginBtn(false)} className={`${!loginBtn ? 'auth-btn-active' : null} auth-btn rounded-r-md`}>Signup</button>
